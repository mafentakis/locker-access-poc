...
| `mqtt-publisher` | `eclipse-temurin:21-jre`   | —                        | `./certs:/certs:ro`                       | `TLS_*` using `rest-server-keystore.p12`, `MQTT_BROKER_URI=ssl://mosquitto:8883`, `LOG_LEVEL`        |
...
- `mqtt-publisher`: `MQTT_BROKER_URI`, `MQTT_CLIENT_ID=mqtt-publisher`, `MQTT_TOPIC_OPENED=...`, `LOCKER_ID`, `COMPARTMENT_ID`; reuses `rest-server-keystore.p12`.
...
1. `./scripts/gen-certs.sh` runs from an empty state and produces, in `./certs/`: `ca.crt`, `ca.key`, and for each of `{broker, rest-server, rest-client}`: `<svc>.crt`, `<svc>.key`, `<svc>-keystore.p12`, plus a single `truststore.p12`. The `mqtt-publisher` service reuses `rest-server-keystore.p12`, and the `mqtt-subscriber` service reuses `rest-client-keystore.p12`.
